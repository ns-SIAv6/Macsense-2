...
android {
    ...
    defaultConfig {
        ...
        // Crash reporting OFF in debug; ON in release when SENTRY_DSN is set.
        buildConfigField("boolean", "CRASH_REPORTING_ENABLED", "false")
        // Sentry DSN — injected from .env / CI secret store.
        buildConfigField("String", "SENTRY_DSN", "\"${findProperty("SENTRY_DSN") ?: ""}\"")

        // Supabase offline-first cloud sync — both keys required to activate.
        // Leave blank in .env.example / CI for offline-only builds.
    ...
    buildTypes {
        release {
            ...
            // Enable Sentry in release builds. Requires SENTRY_DSN set in CI secrets.
