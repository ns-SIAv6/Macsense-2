...
class ExportFactoryTest {
    ...
    fun `applyTempoModulation slows down audio`() {
        ...
        assertEquals((samples.size * 2).toDouble(), output.size.toDouble(), 10.0)
    }
    ...
    fun `applyReverb adds tail to audio`() {
        ...
        // First echo lands one delay line later (120ms = 5292 samples at 44.1kHz).
        val tailEnergy = output.drop(5292).take(1000).sumOf { abs(it) }
